...
async def _apply_customer(
    ...
    record = result.scalar_one_or_none()
    if not record:
        record = CustomerEngagementDaily(
            customer_id=user_id,
            date=date,
            views=0,
            clicks=0,
...
async def _flush_bucket(db: AsyncSession, bucket: datetime) -> None:
    ...
    for event in events:
        ...
        info = aggregates.setdefault(
            product_key,
            {"views": 0, "clicks": 0, "carts": 0, "purchases": 0, "revenue": Decimal("0")},
        )
        ...
        if event.event_type == "view":
            info["views"] += 1
            if customer_id:
                customer_agg.setdefault(customer_id, {"views": 0, "clicks": 0, "carts": 0, "purchases": 0, "points": 0})[
                    "views"
                ] += 1
            ...
            if customer_id:
                bucket_values = customer_agg.setdefault(
                    customer_id,
                    {"views": 0, "clicks": 0, "carts": 0, "purchases": 0, "points": 0},
                )
...
async def record_event(db: AsyncSession, payload: EventCreate) -> Optional[ProductEngagementDaily]:
    bucket = _enqueue_event(payload)
    if bucket is None:
        return None

    await _flush_bucket(db, bucket)
    ...
    if payload.event_type == "purchase" and payload.user_id:
        ...
        await loyalty_service.process_purchase_event(db, payload, _today_utc(payload.timestamp))

    return record


async def get_product_engagement(db: AsyncSession, product_id, day=None):
...
async def get_customer_engagement(db: AsyncSession, user_id, day=None):
...
async def process_hourly_events(db: AsyncSession) -> None:
