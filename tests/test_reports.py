...
def get_sales_report(db: Session, days: int = 30) -> ReportRead:
    ...
    sales_stmt = select(
        ...
        text("variant_id").label("variant_id"),  # <-- Se añade .label()
        # ^^^ --------------------------------- ^^^
        func.sum(text("quantity")).label("units_sold")
    ).select_from(
        text("inventory_movements")  # Especificar la tabla para claridad
    ).where(
        text("type = 'sale'"),
    ...
    stmt = select(
        ...
        sales_stmt, ProductVariant.id == sales_stmt.c.variant_id # <-- Ahora esto funcionará
    ).order_by(
    ...
    # Contamos las transacciones de venta en el período
